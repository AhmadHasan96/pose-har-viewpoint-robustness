...
OUTPUT_CSV = "New_videos/04.05.2026/optitrack_NoOffset_main_bones_xyz.csv"
...
for bone in MAIN_BONES:
    x = data[f"{bone}_Position_X"]
    y = data[f"{bone}_Position_Y"]
    z = data[f"{bone}_Position_Z"]

    # 1) make hip the origin
    x_rel = x - hip_x
    y_rel = y - hip_y
    z_rel = z - hip_z

     # 2) flip Y and Z to match your MediaPipe convention
    out[f"{bone}_X"] = x_rel          # X unchanged
    out[f"{bone}_Y"] = y_rel          # Y unchanged 
    out[f"{bone}_Z"] = -z_rel         # flip Z (for now)
