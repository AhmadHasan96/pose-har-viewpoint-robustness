...
# SCALING (NEW)
# ---------------------------
scale = np.linalg.norm(L_h - R_h, axis=1)
...
L_kn_s = L_kn_c / scale[:, np.newaxis]
R_kn_s = R_kn_c / scale[:, np.newaxis]
...
out_df = pd.DataFrame({
    ...
    "L_kn_x": L_kn_s[:,0], "L_kn_y": -L_kn_s[:,1], "L_kn_z": L_kn_s[:,2],
    "R_kn_x": R_kn_s[:,0], "R_kn_y": -R_kn_s[:,1], "R_kn_z": R_kn_s[:,2],
})




out_df.to_csv("New_vidoes/08.04.2026/Analysis Results/Front/Idle/Hip scaled/MP_Scaled_Idle_Front.csv", index=False)
print("Done ✅")